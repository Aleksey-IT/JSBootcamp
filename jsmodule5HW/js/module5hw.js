...
const getUsersByEyeColor = (arr, color) => {
    return arr.find(i => i.eyeColor === color)
};
...
//  */
const getUsersByGender = (arr, gender) => arr.filter(i => i.gender === 'male').map((i) => i.name);
...
const getInactiveUsers = function getInactiveUsers(arr) {
...
//or
// const getInactiveUsers = arr => arr.filter(i => !i.isActive).map((i) => i.name);
...
const getUserByEmail = (arr, email) => arr.reduce(i => email);
...
const getUsersWithAge = (arr, min, max) => Math.min(min) && Math.max(max);
...
const getTotalBalance = function getTotalBalance(arr) {
    return arr.reduce(function (acc, b) {
        return acc + b.balance;
    }, 0);
};
//or
// const getTotalBalance = arr => arr.reduce((acc, b) => acc + b.balance, 0);
...
const getUsersByFriend = (arr, name) => {
    return arr.filter(el => el.friends.includes(name));
};
